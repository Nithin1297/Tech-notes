...
INSERT INTO sales_data
	(sales_id, region, product_type, sales_amount, sales_date)
VALUES
...
SELECT sales_id, region, product_type, sales_amount,
	NTILE(3) OVER  ( ORDER BY sales_amount DESC) AS [Group Number]
FROM sales_data
...
SELECT sales_id, region, product_type, sales_amount,
case NTILE(3) OVER  ( ORDER BY sales_amount DESC)
when 1 then 'High'
when 2 then 'Medium'
when 3 then 'Low'
end
